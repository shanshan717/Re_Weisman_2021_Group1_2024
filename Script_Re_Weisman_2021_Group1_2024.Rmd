...
keywords          : "Calculate reproducibility, R, Cross-cultural, Mental lif"
wordcount         : "3443"
bibliography: 
  - "/Users/ss/Desktop/Re_Weisman_2021_Group1_2024/Group_1-r-references.bib"
floatsintext      : no
linenumbers       : yes
draft             : no
mask              : no
...
Weisman, K., Legare, C. H., Smith, R. E., Dzokoto, V. A., Aulino, F.,
Ng, E., ... & Luhrmann, T. M. (2021). *Similarities and differences in
concepts of mental life among adults and children in five cultures*.
Nature Human Behaviour, 5(10), 1358-1368. (APA)
...
The study found that cognitive abilities travelled separately from bodily 
sensations among both adults and children in all sites, suggesting that a 
mind–body distinction is common across diverse cultures and present by middle childhood. 
Yet there were substantial cultural and developmental differences in the status of 
social–emotional abilities – as part of the body, part of the mind or a third category unto themselves. 
These findings suggest that while some aspects of mental life may be universal, 
the influences of culture and development significantly shape the understanding of social-emotional abilities [@weisman2021similarities].

### 1.2.2 Main Research Questions and Hypotheses

This study explores how adults and children from different cultural
backgrounds understand concepts of mental life. It hypothesizes that
these understandings have certain universal aspects but may show
significant differences in social-emotional abilities [@weisman2021similarities].

### 1.2.3 Research Results and Conclusions

The study found that adults and children universally distinguish between
physiological sensations and cognitive abilities across all study sites,
indicating that a mind-body distinction is common across cultures.
However, there are significant differences in the classification of
social-emotional abilities among different cultures. These findings
suggest that while some aspects of mental life may be universal,
cultural influences significantly shape the understanding of
...
were recruited from elementary schools [@weisman2021similarities].
...
cosine (rc) calculations to gauge the similarity of factors [@weisman2021similarities].
...
```{r parallel dist adults, fig.width = 8, fig.asp = 0.5, echo=FALSE, warning=FALSE}
...
```{r heatmap adults, fig.width = 9, fig.asp = 0.6, echo=FALSE, warning=FALSE}
...
```{r cong all pairs adults, fig.width = 9, fig.asp = 0.6, echo=FALSE, warning=FALSE}
cong_adults %>%
  mutate_at(#vars(contains("labdescript")),
...
```{r cong cis us base adults, fig.width = 8, fig.asp = 0.5, echo=FALSE, warning=FALSE}
# FIGURE 3
...
```{r cong cis gh base adults, fig.width = 8, fig.asp = 0.5, echo=FALSE, warning=FALSE}
# FIGURE S2
...
```{r cong cis th base adults, fig.width = 12, fig.asp = 0.6, echo=FALSE, warning=FALSE}
# FIGURE S3
cong_plot_fun(cong_df = cong_df_adults, 
...
```{r cong cis ch base adults, fig.width = 10, fig.asp = 0.6, echo=FALSE, warning=FALSE}
# FIGURE S4
cong_plot_fun(cong_df = cong_df_adults, 
              which_country = "China") +
  ylim(min_cong_adults$min_cong, 1)
...
```{r cong cis vt base adults, fig.width = 8, fig.asp = 0.5, echo=FALSE, warning=FALSE}
# FIGURE S5
cong_plot_fun(cong_df = cong_df_adults %>%
                mutate_at(#vars(contains("labdescript")),
                  vars(factor_labdescript_A),
...
### Factor retention: parallel analysis
```{r parallel dist children, fig.width = 8, fig.asp = 0.5, echo=FALSE, warning=FALSE}
## Factor retention: parallel analysis
...
```{r heatmap children, fig.width = 12, fig.asp = 0.6, echo=FALSE, warning=FALSE}
...
```{r cong cis children b, fig.width = 10, fig.asp = 0.55, echo=FALSE, warning=FALSE}
# FIGURE 4
...
```{r alt fig 4, fig.width = 8, fig.asp = 0.5, echo=FALSE, warning=FALSE}
# FIGURE 4
# fig.asp chosen to keep absolute height of y-axis relatively similar across adults and children
cong_df_children %>%
  mutate(region_A = case_when(
...
```{r alt fig 3, fig.width = 10, fig.asp = 0.5, echo=FALSE, warning=FALSE}
# fig.asp chosen to keep absolute height of y-axis relatively similar across adults and children
cong_df_adults %>%
  mutate(region_A = case_when(
    country_A == "US" ~ "SF Bay Area",
...
```{r cong all pairs plot a, fig.width = 18, fig.asp = 0.7, echo=FALSE, warning=FALSE}
...
```{r cong all pairs plot b, fig.width = 14, fig.asp = 0.7, echo=FALSE, warning=FALSE}
# FIGURE 2
cong_lower_lim <- ifelse(min(cong_all_ordered$cong) > -0.05, -0.05, 
                         min(cong_all_ordered$cong))
...
```{r dev comp all sites, fig.width = 12, fig.asp = 0.6, echo=FALSE, warning=FALSE}
...
```{r loadings all samples, fig.width = 12, fig.asp = 0.6, echo=FALSE, warning=FALSE}
# FIGURE 1, version 1
...
```{r dominant factor, fig.width = 12, fig.asp = 0.6, include = F, echo=FALSE, warning=FALSE}
...
```{r loadings all samples v2a, fig.width = 12, fig.asp = 0.6, echo=FALSE, warning=FALSE}
...
```{r loadings all samples v2b, fig.width = 12, fig.asp = 0.6, echo=FALSE, warning=FALSE}
...
```{r, fig.width = 8, fig.asp = 0.5, , include = F, echo=FALSE, warning=FALSE}
d_phi %>%
...
```{r, fig.width = 12, fig.asp = 0.6, include = F, echo=FALSE, warning=FALSE}
d_phi %>%
  ggplot(aes(x = factor_bhm_A, 
             color = country,
             # shape = age_group,
...
![](./Repeatability_figures/table1.png)
![](./Repeatability_figures/table2.png)
![](./Repeatability_figures/table3.png)
![](./Repeatability_figures/table4.png)
![](./Repeatability_figures/table5.png)
![](./Repeatability_figures/table6.png)
![](./Repeatability_figures/table7.png)
![](./Repeatability_figures/table8.png)
![](./Repeatability_figures/table9.png)
![](./Repeatability_figures/table10.png)
![](./Repeatability_figures/table11.png)
![](./Repeatability_figures/table12.png)
![](./Repeatability_figures/table13.png)
![](./Repeatability_figures/table14.png)
![](./Repeatability_figures/table15.png)
![](./Repeatability_figures/table16.png)
![](./Repeatability_figures/table17.png)
![](./Repeatability_figures/table18.png)
